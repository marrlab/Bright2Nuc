...
def quantitative_evaluation(path, data, names):
    ...
    logging.info("The mean absolute error on the normalized dataset is: "
                 + str(np.abs(np.mean(data[names.index("abs_errormap")]))))
    ...
    f.write('\n' + "The mean absolute error on the normalized dataset is: "
            + str(np.abs(np.mean(data[names.index("abs_errormap")]))))
